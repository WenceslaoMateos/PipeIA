% Base de datos representada mediante predicados de la forma pieza(id, bocas)
...
% Establece la compatibilidad entre bocas
compatibles(izq, der).
compatibles(der, izq).
...
% A partir de un extremo queda determinado cual es la posicion en la que algo deberia ubicarse o deberia
% estar ubicado, acorde a la boca del extremo.
